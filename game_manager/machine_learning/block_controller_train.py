...
class Block_Controller(object):
        ...
        if self.mode=="train" or self.mode=="train_sample" or self.mode=="train_sample2":
            ...
            if len(self.replay_memory) < self.replay_memory_size / 10:
                ...
                if self.multi_step_learning:
                    ...
                    #gamma_gpu = self.gamma
                    #gamma_gpu = gamma_gpu.to(device)
                    

                    y_batch = torch.cat(
                   #    tuple(reward if done[0] else reward + gamma_gpu * prediction for done ,reward, prediction in
    ...
    #各列毎の高さの差を計算 (self…このクラスのインスタンス:ボード自体 | board…0 or 1で表された現在のブロックの配置)
    def get_bumpiness_and_height(self,board):        
        
        # ブロックが配置されている箇所をTrueとする配列を作成する
        mask = board != 0

        #mask = np.delete(mask, 9, 1)

        # 各行にブロックがあれば左から見たときに最初にある行を、ない場合はボードのサイズ(上から数えたときの最下層)を、それぞれ与える
        # (上から見たとき、何個先にブロック or 壁が存在するか？)
        invert_heights = np.where(mask.any(axis=0), np.argmax(mask, axis=0), self.height)
        
        # (下から見たとき、どれだけのブロックが積み上がっているか？)
        ...
        #　高さの差分と、その合計値を求める
        diffs = np.abs(currs[:-1] - nexts[:-1])
        total_bumpiness = np.sum(diffs) + heights[-1]
    ...
    def get_holes_diff(self, board):
        ...
        num_holes_diff = num_holes - self.num_holes_prev
        self.num_holes_prev = num_holes
        
        if num_holes_diff < 0:
            num_holes_diff = 0
        return num_holes_diff
    ...
    def step_v2(self, curr_backboard,action,curr_shape_class):
        ...
        hole_num_diff = self.get_holes_diff(board)
        lines_cleared, board = self.check_cleared_rows(board)
        ...
        reward -= self.reward_weight[3] * hole_num_diff
